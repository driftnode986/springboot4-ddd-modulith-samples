...
class RestartRepublishTest {
    ...
    private void leaveOutstandingPublication(JdbcTemplate jdbc, String sku) {
        ...
        jdbc.update("DELETE FROM handled_events");
        jdbc.update("DELETE FROM stocks WHERE sku = ?", sku);
    ...
    private Integer quantityOf(JdbcTemplate jdbc, String sku) {
        return jdbc.queryForObject("SELECT quantity FROM stocks WHERE sku = ?", Integer.class, sku);
    }
